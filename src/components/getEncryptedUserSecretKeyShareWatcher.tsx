...
/**
 * Wait for EncryptedUserSecretKeyShare to become `KeyHolderSigned`.
 *
 * - Uses `ika.getEncryptedUserSecretKeyShareInParticularState(id, 'KeyHolderSigned', { timeout, interval, signal })` if available
 * - Falls back to manual polling via `ika.getEncryptedUserSecretKeyShare(id)`
 * - Displays elapsed time, attempts, last state, progress bar, and supports cancel
 */
export default function EncryptedUserSecretKeyShareWatcher() {
  ...
  async function startWatching() {
    ...
    try {
      ...
      const hasHelper =
        typeof (ika as any)?.getEncryptedUserSecretKeyShareInParticularState === "function";

      const run = hasHelper
        ? () =>
            (ika as any).getEncryptedUserSecretKeyShareInParticularState(
              shareID,
              "KeyHolderSigned",
              { timeout: timeoutMs, interval: intervalMs, signal: controller.signal }
            )
        : () => pollUntilStateFallback({
            fetcher: () => (ika as any).getEncryptedUserSecretKeyShare(shareID),
            expectedState: "KeyHolderSigned",
            timeoutMs,
            intervalMs,
      ...
      const encryptedShare = await run();

      setResult(encryptedShare);
      setPretty(toPrintableJSON(encryptedShare));
      setStatus("success");
    } catch (err: any) {
      if (err?.name === "AbortError") {
        ...
        setStatus("timeout");
        setErrorMsg(err?.message ?? "Timeout waiting for KeyHolderSigned state");
      } else {
        setStatus("error");
  ...
  return (
    <section className="space-y-5">
      <header className="space-y-1">
        <h2 className="text-lg font-semibold">Wait for EncryptedUserSecretKeyShare: KeyHolderSigned</h2>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="space-y-2 sm:col-span-2">
          <Label htmlFor="share-id">EncryptedUserSecretKeyShare ID</Label>
          <Input
            id="share-id"
            placeholder="0x..."
            value={shareID}
      ...
      <div className="flex items-center gap-3">
        <Button onClick={startWatching} disabled={!shareID || status === "polling"}>
      ...
      {pretty && (
        <div className="rounded-xl border p-4 bg-muted/40">
          <div className="mb-2 text-sm text-muted-foreground">EncryptedUserSecretKeyShare (KeyHolderSigned) Result</div>
...
function StatusBadge({
  ...
  const map: Record<typeof status, { label: string; variant?: "default" | "secondary" | "destructive" | "outline" }> = {
    ...
    success: { label: "KeyHolderSigned", variant: "default" },
...
/** Generic fallback manual polling */
