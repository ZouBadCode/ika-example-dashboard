...
export default function GetLatestNetworkEncryptionKey() {
  ...
  return (<section className="space-y-4">
    <h2 className="text-lg font-semibold">Latest Network Encryption Key</h2>
    <CodeBlock code={`// Core SDK call\nconst key = await ika.getLatestNetworkEncryptionKey();`}/>
    <Button onClick={fetchLatest} disabled={loading}>{loading?"Querying...":"Fetch Latest"}</Button>
    {errorMsg && <div className="rounded-md border border-red-300 bg-red-50 p-3 text-sm"><strong>Error:</strong> {errorMsg}</div>}
    {pretty && <div className="rounded-xl border p-4 bg-muted/40">
      <div className="mb-2 text-sm text-muted-foreground">Latest Key:</div>
