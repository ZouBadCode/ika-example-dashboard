...
/**
 * Wait for Partial User Signature to become `Completed`.
 *
 * - Tries `ika.getPartialUserSignatureInParticularState(id, 'Completed', { timeout, interval, signal })` if present
 * - Fallback: manual polling using `ika.getPartialUserSignature(id)`
 */
export default function PartialUserSignatureCompletedWatcher() {
  ...
  async function startWatching() {
    ...
    try {
      ...
      const hasHelper = typeof (ika as any)?.getPartialUserSignatureInParticularState === "function";

      const run = hasHelper
        ? () =>
            (ika as any).getPartialUserSignatureInParticularState(
              partialID,
              "Completed",
              { timeout: timeoutMs, interval: intervalMs, signal: controller.signal }
            )
        : () => pollUntilStateFallback({
            fetcher: () => (ika as any).getPartialUserSignature(partialID),
      ...
      const partialSignature = await run();

      setResult(partialSignature);
      setPretty(toPrintableJSON(partialSignature));
      setStatus("success");
    } catch (err: any) {
      if (err?.name === "AbortError") {
        ...
        setStatus("timeout");
        setErrorMsg(err?.message ?? "Timeout waiting for Partial User Signature to be Completed");
      } else {
        setStatus("error");
  ...
  return (
    <section className="space-y-5">
      <header className="space-y-1">
        <h2 className="text-lg font-semibold">Wait for Partial User Signature: Completed</h2>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="space-y-2 sm:col-span-2">
          <Label htmlFor="partial-id">Partial User Signature ID</Label>
          <Input
            id="partial-id"
            placeholder="0x..."
            value={partialID}
            onChange={(e) => setPartialID(e.target.value.trim())}
          />
        </div>
      ...
      <div className="flex items-center gap-3">
        <Button onClick={startWatching} disabled={!partialID || status === "polling"}>
      ...
      {pretty && (
        <div className="rounded-xl border p-4 bg-muted/40">
          <div className="mb-2 text-sm text-muted-foreground">Partial User Signature (Completed) Result</div>
